package dev.itsu.manabe.api

import android.graphics.Bitmap
import android.graphics.BitmapFactory
import dev.itsu.manabe.api.model.*
import org.jsoup.Jsoup
import java.text.SimpleDateFormat

object Manaba {

    private const val URL_LOGIN_1 =
        "https://idp.account.tsukuba.ac.jp/idp/profile/SAML2/Redirect/SSO?execution=e1s1"
    private const val URL_LOGIN_2 =
    ...
    fun login(userName: String, password: String): Boolean {
        ...
        val relayState =
            parsed.getElementsByAttributeValueContaining("name", "RelayState").first()?.`val`()
        val samlResponse =
            parsed.getElementsByAttributeValueContaining("name", "SAMLResponse").first()?.`val`()
            ...
            .forEach { tr ->
                ...
                tr.getElementsByTag("td").forEachIndexed { index, element ->
                    when (index) {
                        ...
                        3 -> {
                            beginAt = if (element.text()
                                    .isEmpty()
                            ) 0L else format.parse(element.text()).time
                        }
                        4 -> {
                            expiredAt = if (element.text()
                                    .isEmpty()
                            ) 0L else format.parse(element.text()).time
                        }
                    }
    ...
    private fun getCourses(url: String): List<Course> {
        ...
        document.getElementsByClass("courselist")
            ...
            .allElements
            .filter {
                (it.classNames().contains("row0") || it.classNames()
                    .contains("row1")) && it.classNames()
            ...
            .forEach { tr ->
                ...
                var year = 0
                var information = ""
                val seasons = mutableMapOf<String, List<String>>()
                val times = mutableMapOf<String, List<Int>>()
                var teachers = listOf<String>()

                tr.getElementsByTag("td").forEachIndexed { index, element ->
                    when (index) {
                        0 -> {
                            element.getElementsByTag("img")[1]!!
                                .let {
                                    imageUrl = "$URL_HOST${it.attr("src")}"
                                }
                            ...
                            element.getElementsByClass("course-card-status").first()!!
                                ...
                                .forEachIndexed { i, img ->
                                    when (i) {
                                        0 -> isNewsAvailable =
                                            img.attr("src") == "/icon_coursenews_on.png"
                                        1 -> isTestOrQuestionAvailable =
                                            img.attr("src") == "/icon-coursedeadline-on.png"
                                        2 -> isReportAvailable =
                                            img.attr("src") == "/icon-coursegrad-on.png"
                                        3 -> isThreadAvailable =
                                            img.attr("src") == "/icon_coursethread_on.png"
                        ...
                        2 -> {
                            ...
                            val split = information.split(" ")
                            if (split.size > 1) {
                                split.forEachIndexed { indexA, value ->
                                    value.split("、").forEach { value ->
                                        when (indexA) {
                                            0 -> seasons[value.substring(0, 1)] =
                                                value.substring(1)
                                                    .toCharArray().map { it.toString() }.toList()
                                            1 -> times[value.substring(0, 1)] =
    ...
    fun getCourseId(url: String): CourseId? {
        if (!loggedIn) return null

        val document = Jsoup.parse(Http.get(url))
        return CourseId(document.getElementsByClass("coursecode").text())
    }
    ...
    fun getNews(newsUrl: String): News? {
        ...
        return News(
            ...
            document.getElementsByClass("msg-info").first()!!.text().replace("  ", "").replace("投稿者", ""),
    ...
    fun getContent(contentUrl: String): Content? {
        ...
        return Content(
            document.getElementsByClass("contents").first()!!.getElementsByTag("a").first()!!
                .text(),
            contentUrl,
            document.getElementsByClass("pagetitle").first()!!.text(),
            document.getElementsByClass("articletext").first()!!.html(),
            format.parse(
                document.getElementsByClass("contents-modtime").first()!!.text()
                    .replace("更新日時 : ", "")
            ).time,
    ...
    fun getCourseIdFromURL(courseUrl: String): String {
        return courseUrl.split("/").last().split("_")[1]
    }

    fun getImage(url: String): Bitmap? {
        return BitmapFactory.decodeStream(Http.getInputStream(url))
    }
